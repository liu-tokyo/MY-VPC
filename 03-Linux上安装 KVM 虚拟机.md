...
> 1. ⾸先，KVM利⽤ Linux内核的虚拟化功能创建⼀个虚拟化的环境，这个环境被称为虚拟机监控器（Virtual Machine Monitor，VMM）或Hypervisor。
> 2. 然后，KVM使⽤硬件虚拟化扩展（如Intel的VT-x或AMD的AMD-V）来提供对虚拟机的⽀持。这些扩展使得KVM能够在每个虚拟机中运⾏⾃⼰的操作系
> 统，并直接访问宿主机的硬件设备。
> 3. 虚拟机监控器负责在宿主机和虚拟机之间进⾏资源的分配和管理。它通过为每个虚拟机提供虚拟设备（如虚拟CPU、虚拟内存和虚拟磁盘）来模拟硬件
> 环境，使得虚拟机可以独⽴运⾏。
